...
int main() {
    ...
    // Write a 1 MB buffer to the story
    auto story_handle = acquire_ret.second;
    std::string buffer(1024 * 1024, 'A'); // Create a 1 MB buffer filled with 'A'
    story_handle->log_event(buffer);
    std::cout << "Logged a 1 MB event to story '" << story_name << "'." << std::endl;
